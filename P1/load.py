...
def insert_data_reports(cursor, data):
    ...
    for index, row in data.iterrows():
        ...
        if not (cursor.fetchone()):
            ...
            cursor.execute(statement, (row['hid'], row['name'], row['year_qtr'], row['county_name'], row['planning_area'], row['type_cntrl'], row['type_hosp'], row['phone'], row['address'], row['city'], row['zip_code'], row['ceo']))
        else:
            ...
            for key, value in column_names.items():
                ...
                if ((attribute == "" or attribute == " ") and (row[key] != "" or row[key] != " ")):
                    ...
                    cursor.execute(updateStatement, (value, row[key], row['hid']))  
                
        statement = f"INSERT INTO report (year, quarter, start_date, end_date) VALUES (%s, %s, %s, %s);"

        year = row['year_qtr']//10  # Extract the year from the "year_qtr" column
        quarter = row['year_qtr']%10  # Extract the quarter from the "year_qtr" column
        cursor.execute(f"select id from report where year = %s and quarter = %s",(year, quarter))
        res = cursor.fetchone()
        if not (res):
            #print("Inserting into report: ", (year, quarter, row['start_date'], row['end_date']))
            cursor.execute(statement, (year, quarter, row['start_date'], row['end_date']))
            cursor.execute("SELECT LAST_INSERT_ID();")  # Get last inserted id from reports
        ...
        cursor.execute("SELECT LAST_INSERT_ID();")  # Get last inserted id from report_content
        ...
        cursor.execute(statement, (rcid, row['dis_medicare_tradicional'], row['dis_medicare_care'], row['dis_medi_tradicional'], row['dis_medi_care']))
        ...
        cursor.execute(statement, (rcid, row['operational_expenses'], row['professional_expenses']))
        ...
        cursor.execute(statement, (rcid, row['avl_beds'], row['stf_beds'], row['lic_beds']))
...
def insert_data_staff(cursor,data):
    ...
    for index, row in data.iterrows():
        ...
        cursor.execute(f"select * from hospital where id = %s",(row['facility_id'],))
        if (cursor.fetchone()): 
            statement = f"INSERT INTO staff ( hid, productive_hours_per_patient, productive_hours, position) VALUES (%s, %s, %s, %s)"
            cursor.execute(statement, (row["facility_id"],row["productive_hours_per_patient"],row["productive_hours"],row["position"]))
